...
In [Part 1]((https://github.com/adkelley/javascript-to-purescript/tree/master/tut04P1), I covered how to handle side effects in PureScript.  You should recall that a function or expression creates a side effect whenever it modifies some state outside its scope, or it has an observable interaction with its calling function or the outside world.  The core concept is that we don't eliminate side effects because we need them to be able to write useful code.  Instead, we aim to represent them in our code in a way that distinguishes them from our pure computations.  The benefit is that readers or future maintainers of our code will readily understand what side effects we are creating and where we generate them.

I introduced a particular class of side effect, called a 'native' side effect (NSE), which is a side effect that is generated by the runtime system.  Console IO, random number generation, and file IO are all examples of NSEs.  In PureScript, you saw how to use console IO and random number generation in Part 1, and now I’ll show you how to use File IO.  But this is not all you are going to learn.  The primary focus is to sync back up with Brian’s [4th tutorial](https://egghead.io/lessons/javascript-composable-error-handling-with-either) by showing how the `bind` abstraction is used to compose nested `Either`s together in PureScript.  One word of caution - understand that that `bind` in PureScript is equivalent to `chain` in Brian’s JavaScript tutorial.
