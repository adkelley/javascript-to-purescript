...
If you read something that you feel could be explained better, or a code example that needs refactoring, then please let me know via a comment or send me a pull request on [Github](https://github.com/adkelley/javascript-to-purescript/tree/master/tut06).  Finally, If you are enjoying this series then please help me to tell others by recommending this article and/or favoring it on social media

## Symbols, Operations & Laws - Oh My!

I must admit that writing this tutorial was difficult at first.  I kept wondering whether I might turn some readers off by making the connection between programming languages and mathematics.   Why?  Well, there seems to be this meme going around that programming is not math; that you don't need math to program.
...
A Semigroup is a type together with a binary operation `⊕` that satisfies the associativity law.  If we unpack this definition further - a binary operation takes two elements from our type and produces another element of the same type.  Moreover, if you think long enough, you'll see that a binary operation is just a method of function composition.  I like to think of binary operations like concatenation, where we smash two objects together to get a third object.  In PureScript, this is called append, with  `(<>)` its infix operator.  Note that Brian uses `concat` in his examples, but I'll use append in both JavaScript and PureScript to stay consistent.  

Good examples of `(<>)` on Semigroups include addition, multiplication, min, and max.  The associativity law states that given elements a, b and c from our type, then `a <> (b <> c) = (a <> b) <> c`.  Notice I didn't include subtraction!  It is not a Semigroup because it doesn't satisfy the associativity law.  Meaning, `a - (b - c)` is not equal to `(a - b) - c`.   The string type and the binary operation of concatenation is another good example of a Semigroup.   Checking whether the associativity law holds, we find that `("foo" <> "bar") <> "baz"` is "foobarbaz" and `"foo" <> ("bar" <> "baz")` is also "foobarbaz".

Simple, right?  I hope you're now wondering what all the fuss was about.  So with that out of the way, I believe we're ready to present the PureScript equivalents to the JavaScript code snippets from Brian's [tutorial](https://egghead.io/lessons/javascript-combining-things-with-semigroups).

## Examples of Types with Semigroups

Let's stick with append as our binary operation (also known as the method of composition) for the following examples.  Below, we create our first type constructor `Sum` that takes two elements of the same type and adds them together to create another element of the same type.   
...
The JavaScript example is straight forward, but if you're not clear, then please review Brian's [tutorial](https://egghead.io/lessons/javascript-combining-things-with-semigroups) on Semigroups. More interesting (yes, I'm biased) is the PureScript example.  We define our newtype constructor `Sum` and the three type class instances that we use to show, append and prove associativity, respectively.  I covered newtype constructors in my very [first tutorial](https://github.com/adkelley/javascript-to-purescript/tree/master/tut01/src/README.md).  But to elaborate further, they are a special case of algebraic data types that define one constructor only, and that constructor must take exactly one argument.  In this example, the constructor is `Sum`,  and its one argument is the generic type `a`.  I chose a generic type because addition may be performed on multiple types, including integers, floats, and natural numbers.

Next, comes our instance declarations.  First up is the `Show `type class, which we use to tell the PureScript compiler how we want to display our results.  Next, is our `Semigroup` which says how we intend to smash two type elements together to derive a third element of the same type.  In the case of numbers, I chose addition to be our append (i.e., binary) operation.  Also, note that I introduced a type class constraint that we haven't seen until now - `Semiring`.  Here again, we are using a well-defined term from abstract algebra that describes type elements that you can add or multiply together.  I am using it to constrain my generic type `a` to those types that can be added or multiplied, only.   And finally, there is the matter of associativity.  To prove that `Sum` is associative, I derive an instance of the `Eq` type class (i.e., equality) to show in my examples that the left and right sides of the expression `(Sum 1 <> Sum 2) <> Sum 3 == Sum 1 <> (Sum 2 <> Sum 3)` are equal.

Now some of you may think that, compared to the JavaScript or some other dynamic language, this is all too terribly verbose.  In part you are correct, it does take more lines in PureScript to express a Semigroup type.  But again, the goal is to be able to reason about your program. Before you waste one precious clock cycle of CPU on execution, it is better to use the types to their fullest to prove the correctness of your solution.  So if you understand what a Semigroup and Semiring is, along with its laws, then these type declarations take you well on your way to achieving this goal.


### All

Are there other Semigroups that we can define besides `Sum`?  Absolutely!  What about booleans?   For example, `true && false = false` which indicates that we can smash two booleans types together to get a third boolean.  What about the associativity law?  Well, `(true && false) && true == true && (false && true)`, so I think we are onto something.  So let's define our next Semigroup using booleans and call it `All`.  
...
There's not much difference between `All` and `Sum` newtype and instance declarations, so if you're lost then re-read the section above on `Sum` and don't hesitate to leave a question in the comments.  We merely swapped out numbers for booleans, and our append operation has become `&&` instead of `+`.  But there is that new type class constraint `BooleanAlgebra`.  Well, it's just another math term to describe types that behave like boolean values.   I'm down for getting easy wins and, compared to Semigroup and Semiring, I believe the definition of this math term is an easy one to remember. Hallelujah!

### First

And finally, let's make a somewhat odd Semigroup that we'll call `First`.  Remember that Strings are a Semigroup where `"a" <> "b" <> "c" = "abc"`.  But what if our append operation was structured to capture only the first argument?  That `First("a") <> First("b") <> First("c") = First("a")`.  Does that still constitute a Semigroup?  Let's check by applying the associativity law - `(First("a") <> First("b")) <> First("c") == (First("a") <> First("b")) <> First("c") == First("a")`.  Yes, me thinks we have a `Semigroup`, so let's code it up.
...
Here again, there's not much difference between `First` and `Sum` newtype and instance declarations.  We merely swapped out numbers for a generic type, be it string, number, your choice.  And our append operation ignores every argument except for the first.

## Existing Semigroup modules in PureScript

I don't know about you, but I would be terribly disappointed if PureScript didn't have some of these Semigroups already sitting in a module, locked and loaded with all the class instances and type constraints ready to go.  Well, look no further than Data.Monoid.Additive for `Additive x <> Additive y == Additive (x + y)`.  And what about Data.Monoid.Conj for `Conj x <> Conj y == Conj (x && y)`?  Look familiar?  They're just `Sum` and `All` in disguise.  But if you're looking for `First` then you'll have to make that one up on your own.
...
That's all for now. In the next Tutorial we'll go through a few examples of Semigroup definitions, similar to the Either examples in [Tutorial 5](https://github.com/adkelley/javascript-to-purescript/tree/master/tut05).  If you are enjoying this series then please help me to tell others by recommending this article and/or favoring it on social media.  Thank you and till next time!
